# Kaggle Orbit Wars opening-gate anti-meta-meta submission template.
...
def _opening_style_signature_2p(own, neutrals, angular_velocity, player=0):
...
def _response_opening_style_signature_2p(own, neutrals, angular_velocity, player=0):
    if not own or not neutrals:
        return None
    home = own[0]
    home_xy = (_planet_x(home), _planet_y(home))
    ...
    if angular_velocity >= 0.042 and ships >= 20 and production >= 4 and nearest_distance <= 15.0:
        return "defensive"
    return _opening_style_signature_2p(own, neutrals, angular_velocity, player=player)


def _locked_opening_style(state, player, own, enemies, neutrals, signature_fn=_opening_style_signature_2p):
    enemy_owner_count = len({_planet_owner(enemy) for enemy in enemies})
    if int(state.get("step", 0)) == 0:
        _OPENING_STYLE_BY_PLAYER[player] = (
            signature_fn(own, neutrals, float(state.get("angular_velocity", 0.0)), player=player)
...
def _opening_gate_anti_meta_meta_agent(state, player):
    planets = state.get("planets", [])
    own = [planet for planet in planets if _planet_owner(planet) == player]
    enemies = [planet for planet in planets if _planet_owner(planet) not in (-1, player)]
    neutrals = [planet for planet in planets if _planet_owner(planet) == -1]
    ...
    opening_style = _locked_opening_style(state, player, own, enemies, neutrals, signature_fn=_response_opening_style_signature_2p)
    ...
    if opening_style == "rush_then_greedy_short":
        return _rush_policy(state, player, own, enemies) if int(state.get("step", 0)) < 3 else fallback_greedy(state)
    ...
    if opening_style == "defensive_then_field_control_short":
        return _defensive_policy(state, player, own, neutrals) if int(state.get("step", 0)) < 1 else _field_control_policy(state, player, coalition_mode=False)
...
def agent(obs):
    try:
        ...
        moves = _opening_gate_anti_meta_meta_agent(obs, player)
