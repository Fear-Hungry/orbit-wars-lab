...
| --- | ---: | --- | --- |
| `53537753` PGS hold+wave w60s150 | 1228.8 | Melhor recorde. Config operacional correto: `scripts="hold", wave_min_ships=60, wave_start_step=150`. | Pacote atual `artifacts/submission_pgs.tar.gz` validado em 2p/4p all seats: `DONE`, `fallbacks=0`, `timeouts=0`, `fallback_errors=0`. |
| `53542884` PGS hold+wave resubmit | 1150.2 | Mesmo config declarado do recorde, score bem menor. Evidencia insuficiente para atribuir apenas a bug; ha ruido/variancia de LB e wrappers antigos eram menos fortes contra fallback frio. | Wrapper atual usa Producer shadow warm, budget 0.9 e kill-switch apos primeiro timeout. Re-submissao so deve ser interpretada apos estabilizacao do LB. |
| `53542864` PGS wave_s100 | 1145.5 | Local antigo inflou a variante; liga/validacao anteriores tinham riscos de cache/fault/status e de fallback invisivel. | Tarball atual validado sem fallback/timeout. Regua local parcial nao favorece `pgs_wave_s100` contra `pgs_hold`. |
...
| `53433131` OEP robust wrapper | 1182.7 | Wrapper robusto evitou ERROR anterior, mas template OEP ainda podia ocultar fallback antes do hardening. | `artifacts/submission_oep.tar.gz` atual validado: `DONE`, `fallbacks=0`, `timeouts=0`, `fallback_errors=0`. |
...
rtk .venv/bin/python scripts/validate_pgs_tarball.py \
  --tarball artifacts/submission_pgs.tar.gz --seats all --label pgs_holdwave_current

rtk .venv/bin/python scripts/validate_pgs_tarball.py \
  --tarball artifacts/submission_oep.tar.gz --skip-pgs-planner-check \
  --seats all --label oep_current
...
- PGS holdwave atual: `VALIDATION OK`, `fallbacks=0`, `timeouts=0`,
  `fallback_errors=0`, p95 176ms, max 330ms.
- OEP atual: `VALIDATION OK`, `fallbacks=0`, `timeouts=0`,
  `fallback_errors=0`, p95 146ms, max 245ms.
...
A regua longa ainda estava rodando durante este snapshot. Parcial 2p concluido:

| Rank parcial | Bot | Jogos | Win rate | Observacao |
| ---: | --- | ---: | ---: | --- |
| 1 | `pgs_hold` | 480 | 0.610 | Sem faults/bad status nos matches concluidos. |
| 2 | `pgs_holdwave` | 96 | 0.469 | Ainda e o melhor historico de LB. |
| 3 | `pgs_wave_s100` | 96 | 0.406 | Nao confirmado como melhoria. |
| 4 | `oep` | 96 | 0.385 | Pacote atual valido tecnicamente. |
| 5 | `pgs_bigwave` | 96 | 0.323 | Rejeitado como candidato. |
| 6 | `producer` | 96 | 0.281 | Ancora baseline. |

Este parcial **nao substitui** o relatorio final da regua, porque faltavam
pares restantes e jogos 4p.
